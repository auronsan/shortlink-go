...
func FetchAllHandler(bdb *badger.DB, db *badger.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		token := helper.ParseToken(c.Get("Authorization"))
		// Basic Checking without Real time API Token checking
		result, validate := ValidateToken(c)
...
func UpdateHandler(bdb *badger.DB, db *badger.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
...
func DeleteHandler(bdb *badger.DB, db *badger.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
    ...
    err = db.Update(func(txn *badger.Txn) error {
			err = txn.Delete([]byte(post.ShortID))
			it := txn.NewIterator(badger.DefaultIteratorOptions)
			defer it.Close()
			return err
		})
